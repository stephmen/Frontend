...
oc set env deployment/frontend PRODENDPOINT=http://apollo-backend-production.apps-crc.testing
oc set env deployment/frontend endpoint=http://apollo-backend-production.apps-crc.testing
oc expose deployment frontend --port=7777
oc expose service frontend --name=frontend
./createvolumetestenv.sh
oc apply -f config-map-prod.yaml